...
% frequency (500hz)
F = 500;
% target_time = 180; % in seconds
timestamps = linspace(0,target_time,F*target_time);
samples = length(timestamps);

[~, ~, ~, ~, platData] = tdfReadDataPlat(current_filename);

if ~isempty(platData)
    % permute the matrix 
    % matrix format:
    %   rows = samples
    %   columns = variables (Px Py Fz Fy Fz Tz) [m,m,N,N,N,N.m]
    ...
    % config matrices to have variable names
